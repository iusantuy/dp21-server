...
class InvoicePreviewView(View):
    def post(self, request):
        ...
        # month = today.month
        # year = today.year % 100
        # to = data.get("to", "John Doe").strip().title()
        # existing_customer = Invoice.objects.filter(to=to).count()
        # order_no = str(existing_customer + 1).zfill(2)
        # month_order_no = MONTH_ORDER[month]
        
        # invoice_number = data.get("invoice_number", "00/DCP/XI/25")
        data = json.loads(request.body)
        customer_name = data.get("to", "").strip().title()
        generate_invoice_number = generate_invoice_number(customer_name)
        invoice_number = generate_invoice_number
        due_date = data.get("due_date", "2025-11-12")
        ...
        watermarks = [{"top": f"{y}%", "left": f"{x}%"} for y in range(0, 100, 25) for x in range(0, 100, 25)]
        ...
        invoice = {
            ...
            "payment_method": "Bank Central Asia (BCA) an. Fakhri Abdullah Azzam 6640 4166 78",
            "ket": "Mohon lakukan pembayaran satu hari setelah produk diterima dan invoice dtanda tangani",
            "items": items,
            "dp": dp,
            "total": total,
            "total_final": total + 50000,
            "notes": notes,
...
#                 watermarks.append({"top": f"{y}%", "left": f"{x}%"})
...
#             "ket": "Mohon lakukan pembayaran satu hari setelah produk diterima dan invoice dtanda tangani",
#             "dp": 50000,
#             "instagram_handle": "@dolcepapa.21",
#             "items": items,
#             "watermarks": watermarks
#         }

#         invoice["total"] = sum(i["porsi"] * i["harga"] for i in invoice["items"])
#         invoice["total_final"] = invoice["total"] + invoice["dp"]
#         invoice["watermarks"] = watermarks
